...
{
    /// <summary>
    /// FP annotation cleanup runner (delete-by-fullname plan).
    ///
    /// - Uses FpAnnotationDeletionRules.PlanDeletionsFromDrawing(...)
    ...
    public sealed class FpAnnotationCleanupRunner : IDrawingCleanupRunner
    {
            ...
            bool activateEachView = true)
        {
            try
            {
                ...
                Logger.Blue($"[FP.Cleanup] DrawingType={rulesDrawingType}, Shank={shank}, Foot={foot}");
        ...
        private static FpAnnotationDeletionRules.Options BuildCobOptionsFromWedgeDimensions(WedgeData wedge)
        {
        ...
        private static FpAnnotationDeletionRules.DrawingType ResolveCobRulesDrawingType(
        ...
        private static (FpAnnotationDeletionRules.ShankType Shank, FpAnnotationDeletionRules.FootOption Foot)
            ResolveCobShankAndFoot(WedgeData wedge)
        {
            ...
            Logger.Blue($"[FP.Resolve] Wed-Type={wedType ?? "(null)"}, Wed-Foot_Option={footOpt ?? "(null)"}");
        ...
        private static FpAnnotationDeletionRules.ShankType ParseCobShankType(string? wedType)
        {
        ...
        private static FpAnnotationDeletionRules.FootOption ResolveCobFootOption(
            WedgeData wedge, string? rawFootOption)
        {
            ...
            if (baseFoot == FpAnnotationDeletionRules.FootOption.C)
            {
                bool allCbrPositive =
                ...
                if (allCbrPositive)
                {
                    Logger.Info("[FP.Cleanup] Foot rule: raw=SW_C + (CBRA/CBRL/CBRD all > 0) → C_WITH_CBR.");
