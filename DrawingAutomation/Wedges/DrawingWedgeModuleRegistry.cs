...
using WAD.Runner.DrawingAutomation.Wedges.M;
...
public static class DrawingWedgeModuleRegistry
{
    ...
    public static IDrawingWedgeModule Get(
        WedgeType wedgeType)
    {
    ...
    public static IEnumerable<IAnnotationRuleCatalog> GetAnnotationCatalogs()
        => Modules.Values
            .SelectMany(
                module => module.AnnotationCatalogs)
            .GroupBy(
                catalog => catalog.Profile)
            .Select(
                group => group.First());

    private static IReadOnlyDictionary<WedgeType, IDrawingWedgeModule>
        BuildModules()
    {
    ...
    private static void Validate(
        IDrawingWedgeModule module)
    {
        if (module.Behavior is null)
        {
            throw new InvalidOperationException(
                $"{module.WedgeType} has no drawing behavior.");
        }

        if (module.OverlayPositioningRule is null)
        {
            throw new InvalidOperationException(
                $"{module.WedgeType} has no overlay positioning rule.");
        }

        if (module.Profiles is null ||
            module.Profiles.Count == 0)
        {
            throw new InvalidOperationException(
                $"{module.WedgeType} has no drawing profiles.");
        }

        if (module.AnnotationCatalogs is null ||
            module.AnnotationCatalogs.Count == 0)
        {
            throw new InvalidOperationException(
                $"{module.WedgeType} has no annotation catalogs.");
        }

        if (module.AnnotationContextResolver is null)
        {
            throw new InvalidOperationException(
                $"{module.WedgeType} has no annotation context resolver.");
        }

        var duplicateProfileKeys =
            module.Profiles
                .GroupBy(
                    profile => profile.Key)
                .Where(
        ...
        {
            if (profile.Key.WedgeType !=
                module.WedgeType)
            {
                throw new InvalidOperationException(
                    $"Profile '{profile.ProfileName}' belongs to " +
                    $"{profile.Key.WedgeType}, but is registered by " +
                    $"the {module.WedgeType} module.");
            }

            var annotationProfile =
                module.ResolveAnnotationProfile(
                    profile.Key.Subclass,
                    profile.Key.DrawingType);

            if (!module.AnnotationCatalogs.Any(
                    catalog =>
                        catalog.Profile ==
                        annotationProfile))
            {
                throw new InvalidOperationException(
                    $"Profile '{profile.ProfileName}' resolves to " +
                    $"annotation profile '{annotationProfile}', but the " +
                    $"{module.WedgeType} module does not provide it.");
            }
        ...
        DrawingProfile profile)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Profile '{profile.Key}' has no name.");
        }

        if (profile.SheetSelector is null)
        {
            throw new InvalidOperationException(
                $"Profile '{profile.Key}' has no sheet selector.");
        }

        if (profile.BreaklineViews is null)
        {
            throw new InvalidOperationException(
                $"Profile '{profile.Key}' has no breakline policy.");
        }

        if (profile.Scale is null ||
            profile.Scale.Step <= 0.0 ||
            profile.Scale.MinScale <= 0.0 ||
            profile.Scale.MaxScale <
            profile.Scale.MinScale ||
            profile.Scale.FillRatioHeight <= 0.0 ||
            profile.Scale.FillRatioHeight > 1.0)
        {
            throw new InvalidOperationException(
                $"Profile '{profile.Key}' has an invalid scale policy.");
        }
    }
}