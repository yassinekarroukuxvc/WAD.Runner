...
public sealed class FpAnnotationContextResolver : IAnnotationWedgeContextResolver
{
    ...
    public AnnotationWedgeContext Resolve(WedgeData wedge)
    {
        ...
        var froEqualsFr = ResolveFroEqualsFr(facts)
            ? FpAnnotationTraitValues.True
            : FpAnnotationTraitValues.False;
        ...
        return new AnnotationWedgeContext
        {
            Traits = new AnnotationTraitSet(new[]
            {
                ...
                Pair(FpAnnotationTraitNames.FroEqualsFr, froEqualsFr)
            }),
    ...
    private static string ResolveShankToken(WedgeData wedge)
    {
        ...
        return token switch
        {
            "SW_STD" or "STD" => FpAnnotationShankTypes.Std,
            "SW_180REV" or "SW_180_REV" or "180REV" or "180_REV" => FpAnnotationShankTypes.Rev,
            _ => token
        };
    }
    ...
    private static string ResolveFootToken(WedgeData wedge, DrawingWedgeFacts facts)
    {
        ...
        return token switch
        {
            "LW_VG" or "SW_VG" or "VG" => FpAnnotationFootOptions.Vg,
            "LW_G" or "SW_G" or "G" => FpAnnotationFootOptions.G,
            "LW_C" or "SW_C" or "C" => HasCbr(facts)
                ? FpAnnotationFootOptions.CWithCbr
                : FpAnnotationFootOptions.C,
            _ => FpAnnotationFootOptions.FlatOrUnknown
        };
    }
    ...
    private static string ResolveFeedHoleToken(WedgeData wedge)
    {
        ...
        {
            return FpAnnotationFeedHoleTypes.Standard;
        }

        if (token.StartsWith("OVAL", StringComparison.OrdinalIgnoreCase))
            return FpAnnotationFeedHoleTypes.Oval;

        if (token.StartsWith("SLOT", StringComparison.OrdinalIgnoreCase))
            return FpAnnotationFeedHoleTypes.Slot;

        return FpAnnotationFeedHoleTypes.Unknown;
    }
