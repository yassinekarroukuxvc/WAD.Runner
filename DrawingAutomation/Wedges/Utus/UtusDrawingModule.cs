...
using WAD.Runner.DrawingAutomation.Wedges.Utus.Annotations;

namespace WAD.Runner.DrawingAutomation.Wedges.Utus;

public sealed class UtusDrawingModule : IDrawingWedgeModule
{
    ...
    public UtusDrawingModule()
    {
        ...
        {
            DrawingProfileFactory.Create(WedgeType, WedgeSubclass.FG, DrawingType.Production, "UTUS FG Production", ProductionCustomerViews, new[] { "SHEET1" }, DrawingViewNames.SecondaryBreaklineViews),
            DrawingProfileFactory.Create(WedgeType, WedgeSubclass.FG, DrawingType.Customer, "UTUS FG Customer", ProductionCustomerViews, new[] { "SHEET1" }, DrawingViewNames.SecondaryBreaklineViews),
            DrawingProfileFactory.Create(WedgeType, WedgeSubclass.FG, DrawingType.Overlay, "UTUS FG Overlay", OverlayViews, new[] { "OVERLAY" }, DrawingViewNames.NoBreaklineViews),
            DrawingProfileFactory.Create(WedgeType, WedgeSubclass.PGB, DrawingType.Production, "UTUS PGB Production", ProductionCustomerViews, new[] { "SHEET1" }, DrawingViewNames.SecondaryBreaklineViews),
            DrawingProfileFactory.Create(WedgeType, WedgeSubclass.PGB, DrawingType.Customer, "UTUS PGB Customer", ProductionCustomerViews, new[] { "SHEET1" }, DrawingViewNames.SecondaryBreaklineViews),
            DrawingProfileFactory.Create(WedgeType, WedgeSubclass.PGB, DrawingType.Overlay, "UTUS PGB Overlay", OverlayViews, new[] { "OVERLAY" }, DrawingViewNames.NoBreaklineViews)
        });
    }

    public WedgeType WedgeType => global::WAD.Runner.DataManagement.Domain.Wedge.WedgeType.UTUS;
    ...
    public IOverlayViewPositioningRule OverlayPositioningRule { get; } =
        new UtusOverlayViewPositioningRule();
    ...
    public IReadOnlyList<IAnnotationRuleCatalog> AnnotationCatalogs { get; } =
        ...
        {
            new UtusFgProductionAnnotationRules(),
            new UtusFgCustomerAnnotationRules(),
            new UtusFgOverlayAnnotationRules(),
            new UtusPgbProductionAnnotationRules(),
            new UtusPgbOverlayAnnotationRules()
        });

    public IAnnotationWedgeContextResolver AnnotationContextResolver { get; } =
        new UtusAnnotationContextResolver();
        ...
        DrawingType drawingType)
    {
        if (subclass == WedgeSubclass.PGB)
        {
            return drawingType == DrawingType.Overlay
                ? AnnotationCleanupProfile.UtusPgbOverlay
                : AnnotationCleanupProfile.UtusPgbProduction;
        }

        return drawingType switch
        {
            DrawingType.Overlay => AnnotationCleanupProfile.UtusFgOverlay,
            DrawingType.Customer => AnnotationCleanupProfile.UtusFgCustomer,
            _ => AnnotationCleanupProfile.UtusFgProduction
        };
    }
