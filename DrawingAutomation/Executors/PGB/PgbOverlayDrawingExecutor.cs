...
{
    /// <summary>
    /// CKVD PGB Overlay drawing automation.
    /// Uses a dedicated overlay template but the same part template (PGB_OVERLAY config).
    ...
    public static class PgbOverlayDrawingExecutor
    {
            ...
            IEnumerable<AnnotationPositioner.Plan>? plannedDims)
        {
            ...
            Logger.Info("=== WAD ▶ PGB/Overlay (static placement, no autoscale) ===");

            var profile = ProfileRegistry.Get(run.Wedge.Subclass, drawingData.DrawingType);
            Logger.Info($"[Profile] Using {profile.ProfileName} for PGB/Overlay.");
            ...
            // 3c) Bind views to the correct configurations:
            //     - Front + Side → PGB_DRAWING (Production)
            //     - Detail / Section / Top → PGB_OVERLAY (Overlay)
            try
            {
                var model = ds.Model as ModelDoc2;
                if (model == null)
                {
                    Logger.Warn("[3c/9] Model is null; skipping view config binding.");
                }
                else
                {
                    Logger.Info("[3c/9] Bind PGB overlay views to configurations (Front/Side → PGB_DRAWING, others → PGB_OVERLAY)…");
                    ...
                    // Other overlay views → PGB_OVERLAY config
            ...
            {
                Logger.Warn($"[3c/9] Overlay view config binding step failed (continuing): {ex.Message}");
            }
            ...
            // 7c) Delete Front view when VR == 0
            ...
            Logger.Success("PGB/Overlay drawing execution completed.");
        }
