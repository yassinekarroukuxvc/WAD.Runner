...
{
    /// <summary>
    /// COB tolerances planning (pure logic).
    ///
    /// Domain contract:
    ...
    public sealed class CobToleranceRules : IToleranceRuleSet
    {
        public TolerancePlan Build(WedgeData wedge, DrawingType drawingType, WedgeSubclass subclass)
        {
            ...
            if (ra2hPositive)
            {
                ...
                Logger.Info($"[CobToleranceRules] RA2H > 0 → planned tolerances for {ra2hSketch}.");
            }
            ...
            if (slbEnabled)
            {
                ...
                Logger.Info($"[CobToleranceRules] SLB enabled (VBL > 0) → planned tolerances for {slbSketch}.");
            }
            ...
            if (vrPositive)
            {
                ...
                if (vwEqualsW)
                {
                    ...
                    Logger.Info("[CobToleranceRules] VW case 2 (VW == W) → planned VW tolerances for VW_LEFT_case_2_overlay_sketch.");
                }
                else
                {
                    ...
                    // VR_MAX = VR_NOM + VR_UTOL  →  stored as a single "upper bound" value
                    // VR_MIN = VR_NOM - VR_LTOL  →  stored as a single "lower bound" value
                    // These are derived scalar values, not tolerance pairs, so we push them
                    // as individual ToleranceUpdate entries using their computed values directly.
                    AddComputedBoundsMm(updates, wedge, nomKey: "VR",
                        maxTarget: $"VR_MAX@{Case1Sketch}",
                        minTarget: $"VR_MIN@{Case1Sketch}");

                    // VRR_MAX = VRR_NOM + VRR_UTOL
                    // VRR_MIN = VRR_NOM - VRR_LTOL
                    AddComputedBoundsMm(updates, wedge, nomKey: "VRR",
                        maxTarget: $"VRR_MAX@{Case1Sketch}",
                        minTarget: $"VRR_MIN@{Case1Sketch}");

                    Logger.Info($"[CobToleranceRules] VW case 1 (VW != W) → planned W, VW, VR bounds, VRR bounds for {Case1Sketch}.");
                }
            }
            ...
            string ltolTarget)
        {
            ...
            {
                Logger.Warn($"[CobToleranceRules] Missing/invalid tolerance for '{dimKey}' (skipping: {ltolTarget}, {utolTarget})");
                return;
            }
            ...
            Logger.Info($"[CobToleranceRules] {dimKey}: LTOL={ltolMm}mm → {ltolTarget}, UTOL={utolMm}mm → {utolTarget}");
        }
            ...
            string minTarget)
        {
            ...
            {
                Logger.Warn($"[CobToleranceRules] Cannot compute bounds for '{nomKey}': dimensions null.");
                return;
            }
            ...
            if (dim is null)
            {
                Logger.Warn($"[CobToleranceRules] Missing dimension '{nomKey}' for bound computation (skipping: {maxTarget}, {minTarget}).");
                return;
            }

            if (dim.Nominal.Unit != UnitKind.Millimeter)
            {
                Logger.Warn($"[CobToleranceRules] '{nomKey}' nominal unit is {dim.Nominal.Unit} (expected Millimeter).");
                return;
            }
            ...
            Logger.Info($"[CobToleranceRules] {nomKey}: NOM={nom}, UTOL={utol}, LTOL={ltol} → MAX={maxVal} → {maxTarget}, MIN={minVal} → {minTarget}");
        }
