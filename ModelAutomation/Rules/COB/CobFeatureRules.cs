            ...
            HashSet<string> unsuppress)
        {
            ...
            switch (profile)
            {
                ...
                case "std_cut":
                    ...
                    Logger.Info("[CobFeatureRules] Overlay config/profile '" + profile + "' → standard cut state.");
                    break;

                case "non_std_cut":
                    ForceNonStandardCutState(suppress, unsuppress);
                    Logger.Info("[CobFeatureRules] Overlay config/profile 'non_std_cut' → non-standard cut state.");
                    break;
            }
        }
