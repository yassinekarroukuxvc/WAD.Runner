...
namespace WAD.Runner.ModelAutomation.Rules.UTUS
{
    ...
    ///
    /// Current scope:
    /// - UTUS Overlay: push W / FD / T tolerances into overlay sketch parameters.
    ...
    public sealed class UtusToleranceRules : IToleranceRuleSet
    {
        public TolerancePlan Build(WedgeData wedge, DrawingType drawingType, WedgeSubclass subclass)
        {
            ...
            if (ra2hPositive)
            {
                ...
                Logger.Info($"[UtusToleranceRules] RA2H > 0 → planned tolerances for {ra2hSketch}.");
            }
            ...
            if (slbEnabled)
            {
                ...
                Logger.Info($"[UtusToleranceRules] SLB enabled (VBL > 0) → planned tolerances for {slbSketch}.");
            }
            ...
            if (vrPositive)
            {
                ...
                if (vwEqualsW)
                {
                    ...
                    Logger.Info("[UtusToleranceRules] VW case 2 (VW == W) → planned VW tolerances for VW_LEFT_case_2_overlay_sketch.");
                }
                else
                {
                    ...
                    Logger.Info($"[UtusToleranceRules] VW case 1 (VW != W) → planned W, VW, VR bounds, VRR bounds for {Case1Sketch}.");
                }
            }

            Logger.Info($"[UtusToleranceRules] {subclass} Overlay → planned updates={updates.Count} (Shank={shank})");
            ...
            string minTarget)
        {
            ...
            {
                Logger.Warn($"[UtusToleranceRules] Cannot compute bounds for '{nomKey}': dimensions null.");
                return;
            }
            ...
            if (dim is null)
            {
                Logger.Warn($"[UtusToleranceRules] Missing dimension '{nomKey}' for bound computation (skipping: {maxTarget}, {minTarget}).");
                return;
            }

            if (dim.Nominal.Unit != UnitKind.Millimeter)
            {
                Logger.Warn($"[UtusToleranceRules] '{nomKey}' nominal unit is {dim.Nominal.Unit} (expected Millimeter).");
                return;
            }
            ...
            Logger.Info($"[UtusToleranceRules] {nomKey}: NOM={nom}, UTOL={utol}, LTOL={ltol} → MAX={maxVal} → {maxTarget}, MIN={minVal} → {minTarget}");
        }
