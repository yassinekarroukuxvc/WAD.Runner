// ModelAutomation/Rules/FP/FpFeatureRules.cs
using System;
...
{
    /// <summary>
    /// FP feature toggle planning (NO SolidWorks calls, NO rebuild).
    ///
    /// Same behavior as COB, plus one FP-specific mandatory activation layer:
    ///
    /// FP mandatory additions:
    /// - STD shank  -> always unsuppress VW_STD_feature/sketch and SLB_STD_feature/sketch
    /// - 180 shank  -> always unsuppress VW_180_DEG_REV_feature/sketch and SLB_180_DEG_REV_feature/sketch
    ///
    /// FG:
    /// - Apply full FP feature planning logic (mandatory + optional + foot rules),
    ///   same as COB, plus the FP mandatory activation layer.
    ///
    /// PGB:
    ...
    /// </summary>
    public sealed class FpFeatureRules : IFeatureRuleSet
    {
        public ModelRuleRunner.FeaturePlan Build(WedgeData wedge, FeatureRuleContext context)
        {
            ...
            Logger.Info("[FpFeatureRules] Build → start");
            ...
            {
                Logger.Info($"[FpFeatureRules] Subclass=PGB → applying PGB shank-only rules. Shank={shank}");
                ...
                // FP mandatory feature activation layer
                ApplyFpMandatoryFeatures(shank, suppress, unsuppress);
                ...
                {
                    Logger.Info("[FpFeatureRules] Subclass=PGB + Overlay → applying overlay template feature toggles.");
                    BuildOverlayPlan(wedge, shank, suppress, unsuppress, "PGB_LEFT_overlay_sketch");
                    Logger.Info("[FpFeatureRules] PGB Overlay rule → suppress FRO (STD + 180_DEG_REV).");
                }
                ...
                Logger.Success($"[FpFeatureRules] Build(PGB) → done. unsuppress={unsuppress.Count}, suppress={suppress.Count}");
            ...
            if (drawingType == DrawingType.Overlay)
            {
                Logger.Info("[FpFeatureRules] Subclass=FG + Overlay → applying FG overlay template feature toggles.");
            ...
            Logger.Info($"[FpFeatureRules] Parsed → Subclass=FG, Shank={shank}, Foot={foot}");

            BuildFeaturePlanAlignedWithSpec(wedge, shank, foot, fgSuppress, fgUnsuppress);

            // FP mandatory feature activation layer
            ApplyFpMandatoryFeatures(shank, fgSuppress, fgUnsuppress);
            ...
            Logger.Success($"[FpFeatureRules] Build(FG) → done. unsuppress={fgUnsuppress.Count}, suppress={fgSuppress.Count}");
        ...
        // FP mandatory additions
        // --------------------------------------------
        private static void ApplyFpMandatoryFeatures(
            CobShankType shank,
            ...
            HashSet<string> unsuppress)
        {
            ...
            // Always enable the full active-shank VW + SLB set (feature + sketch variants)
            ...
            // Prevent leakage from opposite shank (feature + sketch variants)
            ...
            Logger.Info($"[FpFeatureRules] FP mandatory rule applied for shank={shank}: always enable full VW + SLB set.");
        }
            ...
            HashSet<string> unsuppress)
        {
            ...
            Logger.Info($"[FpFeatureRules] Non-Overlay ({drawingType}) → force suppress: cut_feature, cut_plan_feature");
        }
            ...
            HashSet<string> unsuppress)
        {
            ...
            if (shank == CobShankType.Rev180)
                Logger.Info("[FpFeatureRules] PGB 180_DEG_REV hint: configuration name expected = FP_180_DEG_REV_PGB");
        }
            ...
            string leftOverlaySketch)
        {
            ...
            if (!string.IsNullOrWhiteSpace(leftOverlaySketch))
            {
                if (vrPositive)
                {
                    ...
                    Logger.Info($"[FpFeatureRules] Overlay rule: VR > 0 → suppress '{leftOverlaySketch}'.");
                }
                else
                {
            ...
            if (ra2Positive)
            {
                ...
                Logger.Info("[FpFeatureRules] Overlay rule: RA2 > 0 → suppress PGB_STD_FRONT_overlay_sketch.");
            }
            ...
            if (ra2hPositive)
            {
                ...
                Logger.Info($"[FpFeatureRules] Overlay rule: RA2H > 0 → unsuppress {(isStd ? Ra2hStdFront : Ra2hRevFront)}.");
            }
            else
            {
                suppress.Add(Ra2hStdFront);
            ...
            if (vrPositive)
            {
                ...
                if (vwEqualsW)
                {
                    ...
                    Logger.Info("[FpFeatureRules] Overlay rule: VR > 0 and VW == W → unsuppress VW_LEFT_case_2_overlay_sketch.");
                }
                else
                {
                    unsuppress.Add(VwLeftCase1);
                    suppress.Add(VwLeftCase2);
                    Logger.Info("[FpFeatureRules] Overlay rule: VR > 0 and VW != W → unsuppress VW_LEFT_case_1_overlay_sketch.");
                }
            }
            else
            {
            ...
            if (slbEnabled)
            {
                ...
                Logger.Info($"[FpFeatureRules] Overlay rule: SLB enabled → unsuppress {(isStd ? SlbStdOverlay : SlbRevOverlay)}, suppress matching front overlay sketch and 10BA.");
            }
            else
            {
                suppress.Add(SlbStdOverlay);
            ...
            HashSet<string> unsuppress)
        {
            ...
            if (baIsZero)
                Logger.Info("[FpFeatureRules] Business rule triggered: BA == 0 → suppress 10BA (STD + 180_DEG_REV) and force-enable SLB.");
            ...
            foreach (var opt in optionalBases)
            {
                ...
                Logger.Info($"[FpFeatureRules] Optional '{opt}' enabled={enabled}");
        ...
        private static CobFootOption ResolveFootOption(WedgeData wedge)
        {
            ...
            if (baseFoot == CobFootOption.C)
            {
                ...
                if (allPositive)
                {
                    Logger.Info("[FpFeatureRules] Foot rule: base=C and (CBRA/CBRD/CBRL all > 0) → using C_WithCbr.");
            ...
            HashSet<string> unsuppress)
        {
            ...
            switch (profile)
            {
                ...
                case "std_cut":
                    ...
                    Logger.Info("[FpFeatureRules] Overlay config/profile '" + profile + "' → standard cut state.");
                    break;

                case "non_std_cut":
                    ForceNonStandardCutState(suppress, unsuppress);
                    Logger.Info("[FpFeatureRules] Overlay config/profile 'non_std_cut' → non-standard cut state.");
                    break;
            }
        }
