...
namespace WAD.Runner.ModelAutomation.Rules
{
    ...
    ///
    /// Same baseline behavior as COB, plus FP-specific mandatory rules:
    ///
    /// FP mandatory additions:
    /// - STD shank  -> always unsuppress VW_STD_feature and SLB_STD_feature
    /// - 180 shank  -> always unsuppress VW_180_DEG_REV_feature and SLB_180_DEG_REV_feature
    ///
    /// FG:
    ...
    public sealed class FpFeatureRules : IFeatureRuleSet
    {
            ...
            HashSet<string> unsuppress)
        {
            ...
            // Always enable selected-shank FP mandatory features
            unsuppress.Add(BuildFeatureName("VW", shank));
            unsuppress.Add(BuildFeatureName("SLB", shank));

            // Prevent leakage from opposite shank
            suppress.Add(BuildFeatureName("VW", opposite));
            suppress.Add(BuildFeatureName("SLB", opposite));

            Logger.Info($"[FpFeatureRules] FP mandatory rule applied for shank={shank}: always enable VW + SLB.");
        }
