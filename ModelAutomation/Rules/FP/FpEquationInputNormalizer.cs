// ModelAutomation/Rules/Equations/FpEquationInputNormalizer.cs
using System;
...
{
    /// <summary>
    /// FP-specific dimension normalization (pure logic).
    /// Same behavior as COB:
    /// Adds derived dimensions that are not provided by the DB (e.g., funnel_gap),
    /// and applies FP-specific defaulting rules for missing inputs.
    /// </summary>
    public sealed class FpEquationInputNormalizer : IEquationInputNormalizer
    {
        ...
        public IReadOnlyDictionary<DimensionKey, Dimension> Normalize(WedgeData wedge, DrawingType drawingType)
        {
            ...
            // alpha = FNA/2 (deg)
            // k = BA + RA (deg)
            // funnel_gap = 1/(2*sin(alpha)) * [ FNO * ((1 - tan^2(alpha)*tan^2(k)) / (1 + tan^2(alpha)*tan^2(k))) - H ]
            //
            // If FNO missing/zero -> funnel_gap = 0.0003" = 0.00762 mm
            var funnelGapMm = ComputeFunnelGapMmOrDefault(wedge);

            UpsertLengthMm(result, "funnel_gap", funnelGapMm);

            Logger.Info($"[FpEquationInputNormalizer] funnel_gap = {funnelGapMm} mm");
        ...
        private static void ApplyVraDefaultRule(WedgeData wedge, IDictionary<DimensionKey, Dimension> result)
        {
            ...
            Logger.Info("[FpEquationInputNormalizer] Rule applied: (VW/VRR/VR > 0) and (VRA missing/0) → VRA = 90deg");
        }
