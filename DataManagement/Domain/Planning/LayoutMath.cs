...
internal static class LayoutMath
{
    ...
    // Values (mm/deg) from your Dimension.Nominal (Quantity.Value is decimal → cast to double)
    ...
    // K (mm): prefer domain KValue; fallback to a "K" dimension if present; else 40% TL (clamped to TL/2).
    internal static double Kmm(WedgeData wedge, double TL)
    {
        if (wedge?.KValue is not null)
        {
            var k = (double)wedge.KValue.ValueMm.Value;          // decimal → double
        ...
        {
            var k2 = (double)kd.Nominal.Value;                   // decimal → double
    ...
    // Legacy band length (sheet mm) used by several placements
