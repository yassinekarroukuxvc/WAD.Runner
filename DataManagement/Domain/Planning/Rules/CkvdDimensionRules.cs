...
internal static class CkvdDimensionRules
{
    ...
    {
        if (!ctx.Drawing.Views.TryGetValue(view, out var v) || v is null)
            return defaultMm;

        if (v.Params is not null)
        {
            if (v.Params.TryGetValue("breakline_gap_mm", out var mm))
                return mm;

            if (v.Params.TryGetValue("BreaklineGap", out var mm2))
                return mm2;
        }

        if (v.Metadata is not null)
        {
            if (v.Metadata.TryGetValue("breakline_gap_mm", out var s1) &&
                double.TryParse(s1, out var p1))
                return p1;

            if (v.Metadata.TryGetValue("BreaklineGap", out var s2) &&
                double.TryParse(s2, out var p2))
                return p2;
        }
